...
// (2) 모든 경우의 수를 뽑아내는 완전탐색을 위한 반복문. 이 답안에서 왜 set으로 유닛을 만들고 add함수를 썼는지 고민했는데, 직접 몇 가지 케이스를 계산해본 결과 중복된 값이 유닛에 들어가면 복잡해질 수 있어서 set을 쓴 것 같다. j가 i보다 하나 작아야 하는 이유는 그 전 결과를 활용해야 하는 동적계획법의 개념을 포함하고 있다. 이 문제의 경우에 답이 되는 경우는 i = 4일 때, j = 1,2,3 i - j = 3,2,1의 범위 안에 있다. j = 3, cache[j] = 55 + 5 그리고 i - j = 1 , cache[i-j] =5 를 가지고
